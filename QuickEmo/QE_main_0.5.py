...
from modules.QE_audioUtils import QE_speaker_diarization, QE_assemble, QE_speech_Identify
from modules.QE_VoiceTextModel import QE_Voice_Text_Model
#import wave  ¿?
...
base_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)),"data/models")  #copiado en el sitio correspondiente, creo
...
while 1 : # Infinite listening loop , en realidad vendrá determinado por el loop del GUI
    ...
    interlocutors_labels = QE_speaker_diarization(
                        sampling_rate = RATE, signal = corte , numSpeakers=1, 
                        classifier_all=0, mean_all=0, std_all=0, class_names_all=0, classifier_fm=0, mean_fm=0, std_fm=0, class_names_fm=0, # Load models from avove
                        mid_window=0.4, mid_step=0.05, # defaults are mid_window=2.0  and mid_step=0.2
                        short_window=0.05, lda_dim=0, plot_res=False)   
                        # separa los interlocutores, 
    ...
    #os.  quería hacer aquí lo del input asíncrono ese
