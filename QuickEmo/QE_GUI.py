...
# QuickEmoGUI: ACCESO REMOTO A EQUIPOS POR SSH, SFTP y RDP
...
class QuickEmoGUI():
    ...
    def __init__(self, img_carpeta, iconos):                      
        ...
        self.ventanaPrincipal.title("QuickEmoGUI ")  # Título
      ...
      #  self.ventanaPrincipal.attributes('-fullscreen', True)  # Maximiza ventana completa        
        ...
        barramenu = Menu(self.ventanaPrincipal)
        self.ventanaPrincipal['menu'] = barramenu
        ...
        barraherr = Frame(self.ventanaPrincipal, relief=RAISED,
                          bd=2, bg="#E5E5E5")
        ...
        self.barraest = Label(self.ventanaPrincipal, text=mensaje, 
                              bd=1, relief=SUNKEN, anchor=W)
        self.barraest.pack(side=BOTTOM, fill=X)

        # DEFINIR MENU CONTEXTUAL

        self.menucontext = Menu(self.ventanaPrincipal, tearoff=0)
        ...
        self.ventanaPrincipal.bind("<Control-c>", 
                       lambda event: self.f_conectar())
        self.ventanaPrincipal.bind("<Control-g>", 
                       lambda event: self.f_guardar())
        self.ventanaPrincipal.bind("<Control-q>", 
                       lambda event: self.f_salir())
        self.ventanaPrincipal.bind("<Button-3>", 
    ...
    def f_acerca(self):
        ...
        etiq1.pack(side=TOP, padx=10, pady=10, 
                   ipadx=10, ipady=10)
        etiq2 = Label(marco1, text="QuickEmoGUI "+__version__, 
        ...
        boton1.focus_set()
        acerca.transient(self.ventanaPrincipal)
        self.ventanaPrincipal.wait_window(acerca)
...
def main():
    ...
    if not error1:
        mi_app = QuickEmoGUI(img_carpeta, iconos)
    return(0)
