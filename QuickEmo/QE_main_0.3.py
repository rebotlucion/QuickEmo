...
currentOnes = [] # estructura con los canales de audio a medida que se van registrando, uno por cada interlocutor. 
...
while 1 : # Infinite listening loop , en realidad vendrá determinado por el loop del GUI
    ...
    mensajes, flag_nuevo, currentOnes = QE_speech_Identify(mensajes, current_ones)  #¿se chace una copia de current_Ones? no debería de hacerse, hay 
    ...
    if flag_nuevo:
        emociones = modeloVT.evaluar_emociones(mensajes) # intenta reconocer la emoción que hay en el par (texto palabra, audio palabra) nuevo que hay en 
