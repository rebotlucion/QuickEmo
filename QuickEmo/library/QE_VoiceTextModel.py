...
class QE_Voice_Text_Model:
    ...
    def evaluate(self, mensajes_interlocutores, current):
        ...
        emotions_interlocutors = [] # las emociones ya consensuadas de todos los mensajes de los diferentes interlocutores
        
        for mensajes in mensajes_interlocutores:
            pending_segment = len(mensajes)-current
            emotions = []
            emotions_vector = [] #la emoción identificada para cada uno de los steps que se van a ir dando dentro de un mensaje de un interlocutor

            for i in range(pending_segment-1): # vamos evaluando lo que está pendiente de un solo interlocutor
                ...
                # fusionar la emoción partiendo de lo que se ha conseguido intentificar en voz y de la positividad del texto
                ...
                index_emo = np.argmax(emotions, axis=1)
                emotions_vector.append( index_emo, emotions[index_emo])

            emotions_weights = [0,0,0,0,0,0,0] #¿que emoción predomina en este conjunto de steps?
            z=sum(emotions_vector[:][i:] if i==0)
            for e in emotions_vector:
