...
QE_audioUtils module

Several common audio functions.

Firstly, some of them are the ones developed by:
...
This ussage is done this way in order to modify slightly the functions so it can be avoided working with files and passing as arguments directly arrays

There are other utility functions developed just for this project, at the end of the file 

"""

### pyAudioAnalysis part
...
""" pyAudioAnalysis General utility functions """
...
# Adapted form pyAudioAnalysis: the original one is called "speaker_diarization")
# It should return something like this
# duplas (    audio,    interlocutor(id_interlocutor, género interlocutor, rango edad interlocutor)      )   ¿¿ probabilidades de confianza ??
def QE_speaker_diarization(
    sampling_rate, signal, 
    n_speakers, 
    classifier_all, mean_all, std_all, class_names_all, classifier_fm, mean_fm, std_fm, 
    class_names_fm, # Load models from avove
    mid_window=2.0, mid_step=0.2,
    ...
    signal = audioBasicIO.stereo_to_mono(signal) # eliminar si ya viene en mono como condicion
...
##########################################################################################################################################
    if n_speakers <= 0:
        s_range = range(2, 10) #QE_: Adapt in this case to range 1-10? We are going to use this diarizantion in short windows, 250-500 ms
###########################################################################################################################################
    else:
        s_range = [n_speakers]
...
#. "fusiona" los cortes de cada interlocutor para ir construyendo cada canal. # faltaría el género y la edad
#  devuelve duplas (    audio,    interlocutor(id_interlocutor, género interlocutor, rango edad interlocutor)      )
def QE_assemble(currentOnes, corte):
    #here the process
    # ?
    # Here the retruntv
    # puede ser útil
    ...
    """
    silence_removal pyAudioAnalysis? ojito que los índices cambian al quitar silencios, creo que
    desde ahora no daría problemas pero no se si lo han podido usar antes
    """
    return  0
    #duplas (    audio,    interlocutor(id_interlocutor, género interlocutor, rango edad interlocutor)      ) 
    

# incorpora audios en "actuales" por cada interlocutor a las palabras incompletas, intenta reconocerlas, 
# y si lo consigue, añade a "mensajes" las nuevas palabras en cada interlocutor y devuelve flag_nuevo=true, y además
#  modifica los indexes en current Ones para apuntar a donde hay que seguir haciendo recognition
def QE_speech_Identify(mensajes, currentOnes):
    # Identify words from speech
    #here the process
    # 
    # Here the return
    """
    ...
    https://git-lium.univ-lemans.fr/Larcher/sidepipe
    ...
    """
    return  mensajes, flag_nuevo, currentOnes #¿punteros? y toda esa vaina
